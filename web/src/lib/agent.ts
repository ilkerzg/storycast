...
export async function agentBrief(ctx: AgentContext | null): Promise<string> {
  ...
  add(
    ...
    "",
    "Then check every V shot and the tail, in parallel: take the first, middle and last frame with",
    `\`${P.FRAME_AT}\` \`{video_url, frame_type}\`, then ask \`${P.VISION}\` \`{model: "${P.CHECKER}", reasoning: true, temperature: 0, max_tokens: 1500,`,
    `system_prompt: ${json(P.SHEET_CHECK_SYSTEM)}, image_urls: [model sheet, ...frames], prompt}\` with the prompt below. If the answer has "sheet": true, film the shot again once.`,
    "",
  ...
  add(
    ...
    ...fence(
      ...
      json([
        ...
        { id: "vo-B01", type: "audio", keyframes: [{ timestamp: "{START_MS of each block}", duration: "{NARRATION_MS}", url: "{NARRATION}" }] },
        { id: "music", type: "audio", keyframes: [{ timestamp: 0, duration: "{TOTAL_MS}", url: "{MUSIC}" }] },
      ]),
    ),
    "   One track per block, V and T alike, each with that block's own narration file. The shots' own sound is not used.",
